...
class ActivityStudents(BaseTable):
    ...
    def __createIfNotExist(self):  # Test 2 & Run 1
        try:
            ...
            sql_ = '''CREATE TABLE IF NOT EXISTS {0}({1},PRIMARY KEY({2}){3}) Engine InnoDB;'''.format(
                self.table_name,
                ','.join([' '.join(map(str, col_info)) for col_info in self.meta_info['cols_info'].items()]),
    ...
    def __importSimlatorData(self, rows_info):  # Test3
        try:
            # 存在表则删除重新创建
            rows_info = "INSERT INTO {0} VALUES {1};".format(self.table_name, ','.join(
                ['(' + ','.join(map(str, s_i)) + ')' for s_i in rows_info]))
            # print(rows_info)
        ...
        try:
            ...
            for colName in self.allColsName:
                if colName in ori_row_info:
                    ...
                    raise Exception(
                        'Invaild Value 在 insert 表({0}) =>don`t give me all NotNull value！缺失的NotNull属性为:{1}'.format(
                            self.table_name, colName))
                else:
    ...
    def deleteData(self, condition: str):
        ...
        try:
            ...
            self.db.rollback()
            warnings.warn(
                message='Error when delete rows-info from ' + self.table_name + '!')
    ...
    def selectData(self, condition=''):  # 失败会return None
        '''
        获取满足条件的记录
...
if __name__ == '__main__':  # 在此对本模块functions进行test
    ...
    database = connect(host='127.0.0.1', database='db_djhdbmpt', user='root', password='QA199839199839qa',
                       port=3306)
    cursor = database.cursor()
    test_table_obj = ActivityStudents(db=database, cursor=cursor)
    ...
    test_table_obj.deleteData('user_id = 3 and id =3')
    id_ = random.randint(100, 10000000)
    test_table_obj.insertDataWrap(
        {'user_id': id_,
         'id': 2,
         'role': '参与者_导入失败'
    ...
    print('TESLA', test_table_obj.selectData('user_id =3'))
    test_table_obj.updateData(action='user_id =3', condition='id =0')
    database.close()