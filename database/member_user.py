...
if __name__ == '__main__':  # 在此对本模块functions进行test
    ...
    test_table_obj =MemberUser(db=database,cursor=cursor)
    id_ = random.randint(100, 10000000)
    test_table_obj.insertDataWrap(
        {'user_id': id_,
         'user_name':  '\'xiaoming\'',
         'last_login_time': '\'2009-06-08 23:53:37\'',
         'password':'\'woshiyigmimaniganxin\'',
         'gender_isBoy':1,
         'is_active':1,
         'is_superuser':0,
         }
    )
    test_table_obj.deleteData('user_id = '+str(id_))
    print('SUV',test_table_obj.selectData())
    print('TESLA',test_table_obj.selectData('user_id =3'))
    test_table_obj.updateData(action='user_name =\'修改后的user_name\'',condition='user_id =2')
    database.close()